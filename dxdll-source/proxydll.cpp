...
// D3D8to9 fix: GetFrontBuffer often fails or returns black. Use GetBackBuffer + CopyRects instead.
			IDirect3DSurface8* pBackBufferSource = NULL;
			if (d3dd->GetBackBuffer(0, D3DBACKBUFFER_TYPE_MONO, &pBackBufferSource) == D3D_OK)
			{
				if (d3dd->CopyRects(pBackBufferSource, NULL, frontBuf, NULL) == D3D_OK)
				{
					// Success mimic
					pBackBufferSource->Release();
				...
				if ((res = frontBuf->LockRect(&lockRect, NULL, D3DLOCK_READONLY)) == D3D_OK) {
					...
					if (OpenClipboard(NULL)) {
						EmptyClipboard();
						int imageSize = lockRect.Pitch * resY;
						...
						if (hDib) {
							...
							if (pDib) {
								...
								pHeader->biWidth = resX;
								pHeader->biHeight = resY;
								pHeader->biPlanes = 1;
								pHeader->biBitCount = 32;
								pHeader->biCompression = BI_RGB;
								pHeader->biSizeImage = imageSize;
								BYTE* pDest = pDib + sizeof(BITMAPINFOHEADER);
								BYTE* pSrc = (BYTE*)lockRect.pBits;
								for (int y = 0; y < (int)resY; y++) {
									memcpy(pDest + (resY - 1 - y) * lockRect.Pitch, pSrc + y * lockRect.Pitch, lockRect.Pitch);
								}
								GlobalUnlock(hDib);
								SetClipboardData(CF_DIB, hDib);
								DebugMessage("screenshot: copied to clipboard");
